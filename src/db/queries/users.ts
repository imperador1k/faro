...
export const getUserAnalytics = cache(async () => {
  ...
  const [progress, dailyData, vocabularyList, completedChallenges, mistakes] =
    await Promise.all([
      getUserProgress(),
      db.query.userDailyStats.findMany({
      ...
      db.query.challengeMistakes.findMany({
...
export const createUserProgress = async (
  ...
  const cefrLevels =
    (existingProgress?.cefrLevels as Record<string, string>) || {};
  if (cefrLevel) {
  ...
  createNotification(
    userId,
    "streak",
    "Iniciaste a tua primeira ofensiva! Mantém o ritmo. 🔥",
    "/learn",
  ).catch(console.error);
...
export const updateUserInfo = async (name: string, imageUrl: string) => {
  ...
  await db
    ...
    .set({
      ...
      userImageSrc: imageUrl || "/duo_crying.png",
    })
    .where(eq(userProgress.userId, userId));
};
...
export const refillHearts = async () => {
  ...
  createNotification(
    userId,
    "system",
    "Corações restaurados! Estás pronto para aprender. ❤️",
    "/learn",
  ).catch(console.error);
...
export const checkStreakReset = async () => {
  ...
  if (
    ...
    !currentProgress.lastStreakDate
  )
  ...
  const diffDays = Math.floor(
    (new Date(today).getTime() -
      new Date(currentProgress.lastStreakDate).getTime()) /
      (1000 * 60 * 60 * 24),
  );
