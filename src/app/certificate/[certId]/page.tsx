...
export default async function CertificatePage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-slate-950 flex flex-col items-center pb-20 px-4 sm:px-6 relative overflow-hidden print:bg-white dark:bg-slate-900 print:py-0 print:px-0 font-sans">
      <style
      ...
      <div className="relative w-full max-w-[800px] min-h-[500px] h-auto bg-white dark:bg-slate-900 shadow-2xl rounded-sm p-6 sm:p-8 md:p-10 flex flex-col items-center justify-between text-center print:shadow-none print:w-[297mm] print:h-[210mm] print:max-w-none print:p-12 z-10 mx-auto animate-in zoom-in-95 fade-in duration-1000 border border-stone-200 dark:border-slate-800 print:border-0">
        ...
        <div className="mt-4 sm:mt-6 w-full flex flex-col items-center gap-2 sm:gap-4 relative z-10">
          ...
          <h1
            className="text-2xl sm:text-3xl md:text-4xl font-black text-stone-800 dark:text-slate-100 uppercase tracking-[0.2em] mt-2 sm:mt-3"
        ...
        <div className="flex-1 flex flex-col items-center justify-center relative z-10 my-4 sm:my-6 px-2 sm:px-12 w-full">
          <p
            className="text-stone-500 dark:text-slate-400 italic text-sm sm:text-base md:text-lg mb-2 sm:mb-3"
            style={{ fontFamily: "Georgia, serif" }}
          >
          ...
          <p className="text-stone-600 dark:text-slate-300 text-xs sm:text-sm md:text-base max-w-lg leading-relaxed px-4">
            Por ter concluído com distinção e absoluto mérito a totalidade do
            curso de{" "}
            <strong className="text-stone-800 dark:text-slate-100 font-black">
        ...
        <div className="w-full flex justify-between items-end px-4 sm:px-12 mb-4 sm:mb-6 relative z-10">
          <div className="flex flex-col items-center">
            <span className="text-stone-800 dark:text-slate-100 font-bold text-lg mb-2">
              {formattedDate}
            </span>
      ...
      <div className="mt-20 text-center print:hidden flex flex-col items-center w-full max-w-[1000px] z-10 relative animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-500">
        ...
        <h2 className="text-3xl md:text-5xl font-black text-stone-800 dark:text-slate-100 tracking-tight mb-4 drop-shadow-sm">
          Tu também podes dominar o{" "}
          <span className="text-sky-500">{certificate.course.title}</span>.
        </h2>
        <p className="text-stone-500 dark:text-slate-400 text-lg md:text-xl font-medium max-w-2xl mx-auto mb-12">
          Junta-te ao{" "}
          <strong className="text-stone-800 dark:text-slate-100">
            {certificate.userName}
          </strong>{" "}
          e a outros estudantes numa jornada épica de aprendizagem. É 100%
          gratuito, divertido e viciante!
        </p>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full mb-12">
          <div className="flex flex-col items-center bg-white dark:bg-slate-900 p-6 rounded-3xl border-2 border-stone-200 dark:border-slate-800 border-b-[6px] shadow-sm">
            <div className="w-14 h-14 bg-sky-100 text-sky-500 rounded-2xl flex items-center justify-center mb-4">
              <Rocket className="w-7 h-7" />
            </div>
            <h3 className="text-stone-800 dark:text-slate-100 font-black text-lg uppercase tracking-wider mb-2">
              Rápido & Eficaz
            </h3>
