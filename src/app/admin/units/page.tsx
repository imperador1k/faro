...
export default async function AdminUnitsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-bold text-sm tracking-wide">
              {unitsData.length} UNIDADE{unitsData.length !== 1 ? "S" : ""}{" "}
              ENCONTRADA{unitsData.length !== 1 ? "S" : ""}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            <input
              ...
              className="w-full bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-[4px] rounded-2xl px-4 py-3 text-sm font-bold text-slate-700 dark:text-slate-200 focus:outline-none focus:border-sky-400 focus:border-b-sky-500 transition-colors placeholder:text-stone-400 dark:text-slate-500 dark:text-slate-400 placeholder:font-bold"
              disabled
            />
          </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[24px] shadow-sm border-2 border-stone-200 dark:border-slate-800 border-b-[8px] overflow-hidden mt-6">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-stone-50 dark:bg-slate-950 border-b-2 border-stone-200 dark:border-slate-800 text-[11px] uppercase tracking-[0.2em] text-stone-400 dark:text-slate-500 dark:text-slate-400 font-black">
            ...
            <tbody className="divide-y-2 divide-stone-100">
              {unitsData.map((unit) => (
                <tr
                  key={unit.id}
                  className="hover:bg-stone-50 dark:bg-slate-950 transition-colors group"
                >
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-xs font-bold text-stone-400 dark:text-slate-500 dark:text-slate-400 truncate max-w-[200px]">
                        {unit.description}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5 text-center">
                    <span className="text-sm font-black text-stone-500 dark:text-slate-400">
                      {unit.lessons.length}
                    </span>
                  </td>
                  <td className="px-6 py-5 text-center">
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-100 lg:opacity-50 lg:group-hover:opacity-100 transition-opacity">
                      <Link
                        ...
                        className="p-2 bg-stone-100 dark:bg-slate-800 text-stone-500 dark:text-slate-400 rounded-xl border-2 border-transparent border-b-[4px] border-b-stone-200 dark:border-b-slate-800 hover:-translate-y-1 hover:border-b-[6px] active:translate-y-1 active:border-b-[2px] transition-all"
                      >
                        <svg
        ...
        {unitsData.length === 0 && (
          <div className="p-16 flex flex-col items-center justify-center text-center gap-4">
            <div className="bg-stone-100 dark:bg-slate-800 p-6 rounded-[24px] border-b-[6px] border-stone-200 dark:border-slate-800">
              <Layers className="w-10 h-10 text-stone-300" />
            </div>
            <div>
              <p className="text-slate-600 font-black text-xl uppercase tracking-tight">
                Nenhuma unidade encontrada
              </p>
              <p className="text-stone-400 dark:text-slate-500 dark:text-slate-400 text-sm mt-2 font-bold max-w-sm mx-auto">
                A data ledger está vazia. Começa por criar uma nova unidade para
                povoares o mundo de jogo.
              </p>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between mt-4 mb-12">
        <span className="text-xs font-black text-stone-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest px-2">
          Página 1 de 1
        </span>
