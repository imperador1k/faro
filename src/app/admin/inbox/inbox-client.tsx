...
const CANNED_RESPONSES = [
  {
    command: "/bug_fix",
    text: "Olá! A nossa equipa de engenheiros (e o Marco 🦉) já esmagaram esse bug. Obrigado por reportares! Como agradecimento, adicionámos algumas gemas à tua conta. Continua a aprender!",
  },
  {
    command: "/review_thanks",
    text: "Uau, muito obrigado pelas 5 estrelas! 🌟 É feedback como este que nos dá energia para continuar a construir a melhor app de aprendizagem do mundo. Se tiveres sugestões, não hesites!",
  },
  {
    command: "/more_info",
    text: "Olá! Recebemos o teu ticket, mas precisamos de um pouco mais de contexto. Podes explicar-nos exatamente que passos deste antes do erro acontecer? Obrigado!",
  },
];
...
export function InboxClient({ initialItems }: Props) {
  ...
  const applyTemplate = (templateText: string) => {
    ...
    const newText =
      textParts.join(" ") + (textParts.length > 0 ? " " : "") + templateText;
  ...
  const handleSendReply = async () => {
    if (!selectedTicket || !selectedTicket.userEmail) {
      toast.error(
        "Este ticket não tem um email válido associado para resposta.",
      );
      return;
    }
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center w-full">
                    ...
                    <span
                      className={`text-[10px] uppercase font-bold shrink-0 ml-2 ${item.isUnread ? (isReview ? "text-amber-500" : "text-blue-400") : "text-stone-300"}`}
                    >
                      {new Date(
          ...
          <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full overflow-hidden border-2 border-stone-200 shrink-0 bg-stone-100 flex items-center justify-center">
                  ...
                  !selectedTicket.userImageSrc.endsWith(".svg") ? (
                    <Image
                      src={selectedTicket.userImageSrc}
                      alt=""
                      width={48}
                      height={48}
                      className="w-full h-full object-cover"
            ...
            <div className="flex-1 min-h-0 overflow-y-auto pr-2 custom-scrollbar flex flex-col gap-6">
              ...
              <div className="bg-white border-2 border-stone-200 border-b-6 rounded-2xl p-6 flex flex-col relative shadow-sm shrink-0">
                ...
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full ${selectedTicket.type === "review" ? "bg-amber-400" : "bg-sky-400"}`}
                  ></div>
                  {selectedTicket.type === "review"
                    ? "Avaliação Recebida"
