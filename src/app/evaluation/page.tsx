...
export default function EvaluationPage() {
  ...
  const [lockedQuestion, setLockedQuestion] =
    useState<PlacementQuestion | null>(null);
  ...
  const feedbackActive =
    showFeedback || readingShowFeedback || listeningShowFeedback;
  const feedbackIsCorrect = showFeedback
    ? isCorrect
    : readingShowFeedback
      ? readingIsCorrect
      : listeningIsCorrect;
  ...
  const getNextQuestion = useCallback((): PlacementQuestion | null => {
    ...
    const usedAtLevel = grammarAnswers.filter(
  ...
  const handleGrammarVerify = () => {
    ...
    pendingContinue.current = () => {
      ...
      if (answered >= GRAMMAR_QUESTION_COUNT) {
        ...
        startTransition(async () => {
          const exercises = await generateComprehension(
            estimated,
            targetLanguage,
            "reading",
          );
  ...
  const handleReadingVerify = () => {
    if (selectedOption === null || readingShowFeedback || !currentReadingQ)
      return;

    const correct =
      currentReadingQ.options[selectedOption]?.is_correct || false;
  ...
  const currentListeningQ =
    currentListeningEx?.questions[listeningQIdx] || null;
  ...
  const handleListeningVerify = () => {
    if (selectedOption === null || listeningShowFeedback || !currentListeningQ)
      return;

    const correct =
      currentListeningQ.options[selectedOption]?.is_correct || false;
    ...
    pendingContinue.current = () => {
      ...
      if (nextQIdx < (currentListeningEx?.questions.length || 0)) {
        ...
        if (nextExIdx < listeningExercises.length) {
          ...
          startTransition(async () => {
            const topic = await generateWritingTopic(
              estimatedLevel,
              targetLanguage,
            );
  ...
  const handleSubmitWriting = () => {
    ...
    startTransition(async () => {
      const result = await gradeWritingOutput(
        writingText,
        estimatedLevel,
        targetLanguage,
      );
      ...
      setPhaseResults((prev) => ({
        ...prev,
        writing: { score: 0, feedback: result.feedback },
      }));
  ...
  const getProgress = (): number => {
    ...
    if (phase === "grammar")
      return 5 + (questionsAnswered / GRAMMAR_QUESTION_COUNT) * 20;
    if (phase === "reading") {
      const totalReadingQs = readingExercises.reduce(
    ...
    if (phase === "listening") {
      const totalListeningQs = listeningExercises.reduce(
        (sum, ex) => sum + ex.questions.length,
        0,
      );
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 relative flex flex-col font-sans overflow-x-hidden">
            ...
            {phase !== "welcome" ? (
              <div className="flex-1 relative flex items-center gap-4 sm:gap-6">
                ...
                <div className="hidden md:flex shrink-0 w-14 h-14 sm:w-16 sm:h-16 bg-white dark:bg-slate-900 rounded-[1.25rem] border-2 border-slate-200 dark:border-slate-800 border-b-[6px] items-center justify-center shadow-xl z-10 relative group">
                  ...
                  />
                  {phase === "grammar" && (
                    <GraduationCap className="h-8 w-8 text-[#58CC02] stroke-[2.5]" />
                  )}
                  {phase === "reading" && (
                    <BookOpen className="h-8 w-8 text-[#1CB0F6] stroke-[2.5]" />
                  )}
                  {phase === "listening" && (
                    <Headphones className="h-8 w-8 text-[#CE82FF] stroke-[2.5]" />
                  )}
                  {phase === "writing" && (
                    <PenTool className="h-8 w-8 text-[#FF9600] stroke-[2.5]" />
                  )}
                </div>
      ...
      <div className="flex-1 relative flex flex-col p-4 sm:p-8 pt-6 sm:pt-10 w-full max-w-7xl mx-auto z-10">
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -left-32 top-20 h-96 w-96 rounded-full bg-[#58CC02]/10 blur-3xl"
        />
        <div
          aria-hidden="true"
          className="pointer-events-none absolute -right-32 top-1/2 h-96 w-96 -translate-y-1/2 rounded-full bg-[#1CB0F6]/10 blur-3xl"
        />
        ...
        <div className="relative w-full max-w-6xl mx-auto flex flex-col gap-6 pb-6 sm:pb-10 pt-2">
                  ...
                  {[
                    {
                      ...
                      iconBg: "bg-[#58CC02]",
                      glow: "hover:shadow-[0_8px_30px_rgba(88,204,2,0.25)]",
                      ring: "hover:border-[#58CC02]/60",
                    },
                    {
                      icon: BookOpen,
                      label: t("reading"),
                      ...
                      iconBg: "bg-[#1CB0F6]",
                      glow: "hover:shadow-[0_8px_30px_rgba(28,176,246,0.25)]",
                      ring: "hover:border-[#1CB0F6]/60",
                    },
                    {
                      icon: Headphones,
                      label: t("listening"),
                      ...
                      iconBg: "bg-[#CE82FF]",
                      glow: "hover:shadow-[0_8px_30px_rgba(206,130,255,0.25)]",
                      ring: "hover:border-[#CE82FF]/60",
                    },
                    {
                      icon: PenTool,
                      label: t("writing"),
                      ...
                      iconBg: "bg-[#FF9600]",
                      glow: "hover:shadow-[0_8px_30px_rgba(255,150,0,0.25)]",
                      ring: "hover:border-[#FF9600]/60",
                    },
                  ].map(
                    (
                      {
                      ...
                      >
                        ...
                        <div
                          className={cn(
                            "w-12 h-12 rounded-2xl flex items-center justify-center border-b-[4px] shadow-md bg-gradient-to-b",
                            bg,
                            border,
                          )}
                        >
                  ...
                  {[
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center">
                        <span className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-tighter hidden sm:block">
                          {label}
                        </span>
                        <span className="text-xs sm:text-sm font-black text-slate-600 mt-1">
                          {level}
                        </span>
                      </div>
                    </motion.div>
          ...
          {phase === "grammar" && activeQuestion && (
            ...
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <div
                  className={cn(
                    "px-4 py-2 rounded-xl text-sm font-black border-2 border-b-4 shadow-sm",
                    `bg-gradient-to-r ${LEVEL_COLORS[currentLevel]} border-blue-500 text-white`,
                  )}
                >
                  {currentLevel}
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {activeQuestion.options.map((option, idx) => {
                  ...
                  if (isSelected) {
                    optionStyle =
                      "border-[#1899D6] border-b-[#1899D6] bg-[#DDF4FF] text-[#1CB0F6] active:translate-y-[6px] active:border-b-[2px] ring-2 ring-[#1CB0F6]/20";
                  }

                  if (showFeedback) {
                    if (isCorrectOption)
                      optionStyle =
                        "border-[#58CC02] border-b-[#46A302] bg-[#D7FFB8] text-[#58CC02] scale-[1.02] shadow-xl z-10 border-b-[8px]";
                  ...
                  return (
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          isSelected && !showFeedback
                            ? "bg-[#1CB0F6] border-[#0092d6] text-white"
                      ...
                      <span className="text-lg flex-1 leading-snug">
                        {option.text}
                      </span>
                      {showFeedback && isCorrectOption && (
                        <CheckCircle2 className="h-7 w-7 shrink-0 text-green-500 absolute right-4 drop-shadow-md" />
                      )}
                      {showFeedback && isSelected && !isCorrectOption && (
          ...
          {phase === "reading" && currentReadingEx && currentReadingQ && (
            <div
              className="animate-in fade-in slide-in-from-right-4 duration-300"
              key={`r-${readingExIdx}-${readingQIdx}`}
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  return (
                    ...
                    >
                      <span
                        className={cn(
                          "shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-base sm:text-lg font-black border-2 border-b-[4px] transition-all group-active:border-b-[0px] group-active:translate-y-[4px]",
                          isSelected && !readingShowFeedback
                            ? "bg-[#1CB0F6] border-[#0092d6] text-white"
                            : readingShowFeedback && isCorrectOption
                              ? "bg-green-500 border-green-600 text-white"
                              : readingShowFeedback &&
                                  isSelected &&
                                  !isCorrectOption
          ...
          {phase === "listening" && currentListeningEx && currentListeningQ && (
            <div
              className="animate-in fade-in slide-in-from-right-4 duration-300"
              key={`l-${listeningExIdx}-${listeningQIdx}`}
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
              ...
              <div className="text-lg font-bold text-slate-800 mb-4">
                <InteractiveText
                  text={currentListeningQ.question}
                  language={targetLanguage}
                />
              </div>
                  ...
                  return (
                    ...
                    >
                      <span
                        className={cn(
                          "shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center text-base sm:text-lg font-black border-2 border-b-[4px] transition-all group-active:border-b-[0px] group-active:translate-y-[4px]",
                          isSelected && !listeningShowFeedback
                            ? "bg-[#1CB0F6] border-[#0092d6] text-white"
                            : listeningShowFeedback && isCorrectOption
                              ? "bg-green-500 border-green-600 text-white"
                              : listeningShowFeedback &&
                                  isSelected &&
                                  !isCorrectOption
                                ? "bg-red-500 border-red-600 text-white"
                                : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-400 group-hover:border-slate-300 group-hover:text-slate-500 dark:text-slate-400",
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-20">
                ...
                <div className="lg:col-span-5 flex flex-col gap-6">
                    ...
                    <div className="relative z-10 w-48 h-48 sm:w-56 sm:h-56 mb-6">
                      <svg className="w-full h-full transform -rotate-90">
                        ...
                        <motion.circle
                          ...
                          fill="transparent"
                          stroke={
                            finalLevel.startsWith("A")
                              ? "#58CC02"
                              : finalLevel.startsWith("B")
                                ? "#1CB0F6"
                                : "#8b5cf6"
                          }
                          strokeWidth="12"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-6xl sm:text-7xl font-black text-slate-800 tracking-tighter">
                          {finalLevel}
                        </span>
                        <span className="text-xs sm:text-sm font-black text-slate-400 uppercase tracking-[0.2em]">
                          {LEVEL_LABELS[finalLevel] || "Level"}
                        </span>
                      </div>
                      <motion.div
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Link
                      href="/practice"
                      className="flex flex-col items-center justify-center gap-3 py-6 bg-[#58CC02] border-2 border-[#46a302] border-b-8 rounded-[2rem] font-black text-white hover:brightness-110 transition-all active:translate-y-1 active:border-b-4 group shadow-lg shadow-green-100"
                    >
                ...
                <div className="lg:col-span-7 flex flex-col gap-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {[
                      ...
                      ({ icon: Icon, label, detail, score, color, bg }, i) => (
                        <motion.div
                          key={label}
                          initial={{ y: 20, opacity: 0 }}
                        ...
                        >
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <div className={cn("text-xl font-black", color)}>
                              {score}%
                            </div>
                          </div>
      ...
      {feedbackActive &&
        (() => {
          ...
          if (phase === "grammar" && activeQuestion) {
            ...
            evalUserAnswer =
              activeQuestion.options[selectedOption ?? 0]?.text || "";
            evalCorrectAnswer =
            ...
            evalUserAnswer =
              currentReadingQ.options[selectedOption ?? 0]?.text || "";
            evalCorrectAnswer =
              currentReadingQ.options.find(
                (o: { text: string; is_correct: boolean }) => o.is_correct,
              )?.text || "";
          } else if (phase === "listening" && currentListeningQ) {
            evalQuestion = currentListeningQ.question;
            evalUserAnswer =
              currentListeningQ.options[selectedOption ?? 0]?.text || "";
            evalCorrectAnswer =
              currentListeningQ.options.find(
                (o: { text: string; is_correct: boolean }) => o.is_correct,
              )?.text || "";
          }
          ...
          return (
            <div
              className={cn(
                "fixed bottom-0 left-0 right-0 z-above-modal animate-in slide-in-from-bottom duration-300",
                feedbackIsCorrect
                  ? "bg-green-100 border-t-2 border-green-300"
                  : "bg-red-100 border-t-2 border-red-300",
              )}
            >
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div
                      className={cn(
                        ...
                        feedbackIsCorrect ? "bg-green-500" : "bg-red-500",
                      )}
                    >
                      {feedbackIsCorrect ? (
                    ...
                    <div className="min-w-0">
                      ...
                      <p
                        className={cn(
                          "text-sm truncate",
                          feedbackIsCorrect ? "text-green-600" : "text-red-600",
                        )}
                      >
                        {feedbackIsCorrect
                          ? t("good_job_message")
