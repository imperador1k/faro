...
export const LessonClient = ({
  ...
  const [activeIndex, setActiveIndex] = useState(() => {
    if (initialQ) {
      ...
      if (
        !isNaN(index) &&
        index >= 0 &&
        index < initialLesson.challenges.length
      ) {
        return index;
      }
    }
    const incompleteIndex = initialLesson.challenges.findIndex(
      (c) => !c.completed,
    );
  ...
  useEffect(() => {
    if (
      ...
      currentChallenge.challengeOptions.length > 0
    ) {
      const left = currentChallenge.challengeOptions
        .filter((o) => o.correct)
        .sort(() => Math.random() - 0.5);
      const right = currentChallenge.challengeOptions
        .filter((o) => !o.correct)
        .sort(() => Math.random() - 0.5);
  ...
  const { playAudio, playMixedSpeech, isPlaying, playingText } =
    useTTS(languageCode);
  ...
  const handleSelect = (optionId: number) => {
    ...
    if (!isAudioMuted) {
      ...
      if (selectedOpt?.text)
        playAudio(selectedOpt.text, 0.9, selectedOpt.audioLang || languageCode);
    }
  };
  ...
  const handleCheck = () => {
    ...
    if (isInsert || isDictation) {
      ...
      if (result.isCorrect) {
        ...
        if (!isClinic) {
          startTransition(() => {
            onChallengeWrong(currentChallenge.id).then((result) => {
              if ("message" in result && !result.success) {
                toast.error(result.message);
                return;
              }
              if (!result.shieldUsed && result.hearts !== undefined) {
    ...
    startTransition(() => {
      ...
      completeAction()
        .then((res) => {
          ...
          if ((res as any)?.streakExtended) {
            setStreakDays((res as any).streak ?? 0);
            ...
            >
              ...
              >
                <h2 className="text-3xl font-black text-slate-800 tracking-tight leading-tight -mt-4 px-2">
                  Vais mesmo{" "}
                  <span className="text-rose-500 italic">desistir</span> agora?!
                  😭
                </h2>
          ...
          <div className="mx-auto flex w-full max-w-[1140px] min-h-full flex-col items-center justify-center gap-y-6">
            ...
            <div className="flex flex-col gap-6 mb-6 items-center justify-center mt-4">
              <div className="flex items-center gap-4">
                <Button
                  variant="ghost"
                  className="bg-[#1CB0F6] border-b-[8px] border-[#0092d6] w-20 h-20 rounded-full shadow-sm transition-all active:translate-y-2 active:border-b-0 hover:bg-[#159fdf] p-0"
                  onClick={() =>
                    playAudio(
                      currentChallenge.question,
                      0.9,
            ...
            {currentChallenge.type === "DICTATION" ? (
              <div className="w-full max-w-[600px] flex flex-col items-center gap-6">
                ...
                <input
                  ...
                  className={cn(
                    ...
                    status === "wrong" &&
                      "border-rose-400 bg-rose-50 text-rose-700 border-b-2 translate-y-0.5",
                  )}
                />
              ...
              <MatchGrid
                ...
                onSelect={(optId) => {
                  if (matchedIds.includes(optId) || wrongMatchFlash.length > 0)
                    return;
                    ...
                    if (isPair) {
                      ...
                      if (newMatched.length === 8) {
                        ...
                        if (isClinic) {
                          ...
                          onChallengeComplete(currentChallenge.id).then(
                            (res) => {
                              if ("message" in res && !res.success) {
                                toast.error(res.message);
                                return;
