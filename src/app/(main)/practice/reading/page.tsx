            ...
            <section className="bg-white dark:bg-slate-900 rounded-[2rem] border-2 border-stone-200 dark:border-slate-800 border-b-8 p-6 md:p-10 flex flex-col h-full shadow-sm relative transition-all hover:border-b-[10px] hover:-translate-y-1 hover:mb-[-2px]">
              ...
              <div className="overflow-y-auto pr-2 md:pr-4 scrollbar-thin scrollbar-thumb-stone-200 scrollbar-track-transparent h-full pb-4 pt-2">
                <article className="prose prose-stone lg:prose-lg max-w-none">
                  ...
                  <div className="text-justify font-serif text-stone-700 dark:text-slate-200 leading-relaxed md:leading-loose text-lg">
        ...
        <div className="flex flex-col gap-6 w-full lg:pb-12 h-max">
          <div className="bg-stone-100/80 p-2 rounded-[1.5rem] border-2 border-stone-200 dark:border-slate-800 flex items-center justify-between shadow-inner">
            {[
              ...
              const isActive =
                currentStep === step.id ||
                (step.id === "mcq" && currentStep === "review");
              const isCompleted =
                (step.id === "mcq" &&
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-white dark:bg-slate-900 text-emerald-500 shadow-sm border-2 border-stone-200 dark:border-slate-800 border-b-4 translate-y-[-2px]"
                      : isCompleted
                        ? "text-emerald-400 bg-emerald-50/50"
          ...
          {(currentStep === "mcq" || currentStep === "review") && examData && (
            <div className="animate-in fade-in slide-in-from-right-4 space-y-6">
              ...
              {examData.questions?.map((q, qIdx) => (
                <div
                  key={qIdx}
                  className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] border-2 border-stone-200 dark:border-slate-800 shadow-sm space-y-6 shrink-0 transition-all hover:border-b-4 hover:-translate-y-0.5"
                >
                  <h3 className="font-black text-stone-700 dark:text-slate-200 text-lg md:text-xl flex items-start gap-4">
                  ...
                  <div className="space-y-3 pl-0 md:pl-14">
                    {q.options.map((opt, oIdx) => {
                      ...
                      if (showMcqResults) {
                        if (isCorrect)
                          style =
                            "border-emerald-500 bg-emerald-100/50 border-b-emerald-600 text-emerald-800 font-bold border-b-4 translate-y-[-2px]";
                        else if (isSelected && !isCorrect)
                          style =
                            "border-red-400 bg-red-50 border-b-red-500 text-red-800 font-bold border-b-2 translate-y-0";
                        else
                          style =
                    ...
                    <>
                      <div className="relative">
                        ...
                        {userEssay.trim().length > 0 && (
                          <div className="absolute bottom-6 right-6 hidden md:flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 rounded-[1rem] border-2 border-stone-200 dark:border-slate-800 font-black text-stone-500 dark:text-slate-400 text-xs tracking-widest uppercase shadow-sm">
                        ...
                        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 mt-4 space-y-6">
                          <div
                            className={cn(
                              ...
                              essayFeedback.score >= 80
                                ? "border-green-300 bg-green-50"
                                : essayFeedback.score >= 50
                              ...
                              <div className="rounded-[1.5rem] bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 p-6 shadow-sm">
                                ...
                                <ul className="flex flex-col gap-3">
                                  {essayFeedback.strengths.map((s, i) => (
                                    <li
                                      key={i}
                                      className="text-stone-600 dark:text-slate-300 font-medium text-sm md:text-base flex items-start gap-2"
                                    >
