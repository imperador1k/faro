...
export default function SpeakingPracticePage() {
  ...
  const getMainButtonIcon = () => {
    if (status === "recording")
      return <Pause className="h-6 w-6 fill-current" />;
  ...
  if (isGeneratingPrompt) {
    return (
      <AILoadingScreen
        message="A preparar Cenário Falado..."
        submessage="A IA está a calibrar o teu parceiro de conversa"
      />
    );
  }
  return (
              ...
              {promptData?.rules && promptData.rules.length > 0 && (
                <div className="mt-2 pt-4 border-t-2 border-stone-100 w-full flex flex-col gap-3">
                  <span className="text-xs font-black tracking-widest uppercase text-amber-500 flex items-center gap-2">
                    <Target className="w-4 h-4 md:w-5 md:h-5" /> Regras de
                    Conversa
                  </span>
