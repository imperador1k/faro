                ...
                {isPlaying ? (
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isPaused
                        ? "bg-emerald-500 border-b-emerald-600 hover:bg-emerald-400"
                        : "bg-indigo-500 border-b-indigo-600 hover:bg-indigo-400",
                    )}
                  >
                ...
                >
                  ...
                  {inputMode === "voice" ? (
                      ...
                      <button
                        onClick={() => {
                          if (isRecording && activeQuestionIndex === idx) {
                            (
                              recognitionRef.current as { stop: () => void }
                            )?.stop();
                          } else {
                        ...
                        className={cn(
                          "h-16 w-16 rounded-2xl shrink-0 border-2 border-transparent transition-all flex items-center justify-center text-white",
                          isRecording && activeQuestionIndex === idx
                            ? "bg-rose-500 hover:bg-rose-600 shadow-inner border-b-0 translate-y-1 animate-pulse"
                            : "bg-indigo-500 hover:bg-indigo-600 border-b-4 border-b-indigo-700 active:border-b-0 active:translate-y-1",
                        )}
                        disabled={
                          !hasAudioFinished ||
                          (isRecording && activeQuestionIndex !== idx)
                    ...
                    <Textarea
                      ...
                      disabled={
                        !hasAudioFinished || isGenerating || isAnalyzing
                      }
                    />
                  )}
                </div>
                  ...
                  <div className="mt-6 pt-6 border-t-2 border-indigo-200 flex flex-col gap-4">
                    ...
                    <ul className="flex flex-col gap-3">
                      {feedback.missedPoints.map((point, idx) => (
                        <li
                          key={idx}
                          className="flex items-start gap-3 bg-indigo-50 border-2 border-indigo-100 text-indigo-800 font-bold text-sm md:text-base px-5 py-3 rounded-xl shadow-sm"
                        >
