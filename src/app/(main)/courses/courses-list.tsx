...
const languageData: Record<
  string,
  { flag: string; gradient: string; greeting: string; nativeName: string }
> = {
  Português: {
    flag: "🇵🇹",
    ...
    <div className="space-y-16">
      ...
      {Object.entries(groupedCourses).map(([languageLabel, langCourses]) => {
        const data = getLanguageData(
          langCourses[0]?.title || languageLabel,
          languageLabel,
        );
              ...
              {langCourses.map((course) => {
                ...
                return (
                  <button
                    ...
                    disabled={isPending}
                    className={cn(
                      "group relative overflow-hidden bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-[8px] rounded-[32px] p-6 lg:p-8 flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-1 hover:border-b-[10px] active:translate-y-1 active:border-b-[2px] cursor-pointer shadow-sm",
                      (isSelected || isActive) &&
                        "border-green-400 !border-b-green-500 shadow-sm shadow-green-500/10",
                      isPending &&
                        "opacity-50 cursor-not-allowed transform-none hover:shadow-none hover:border-b-[8px]",
                    )}
                  >
                      ...
                      {course.imageSrc && course.imageSrc.startsWith("http") ? (
                        <div className="relative w-24 h-24 z-10 group-hover:scale-110 transition-transform duration-300">
                          <Image
                            ...
                            alt={course.title}
                            fill
                            className="object-cover drop-shadow-xl rounded-[20px] aspect-square"
                          />
                        </div>
                      ) : (
                    ...
                    <div className="flex flex-col items-center gap-3 w-full mb-8 z-10">
                      <h3 className="font-black text-slate-700 dark:text-slate-200 text-3xl uppercase tracking-tight group-hover:text-blue-500 transition-colors">
                        {course.title}
                      </h3>
