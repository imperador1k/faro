...
async function CoursesData() {
  ...
  const activeCourse = courses.find(
    (c) => c.id === userProgress?.activeCourseId,
  );
          ...
          <Link href="/learn" className="block w-full" prefetch>
            <div className="w-full bg-white dark:bg-slate-900 rounded-[32px] border-2 border-slate-200 dark:border-slate-800 border-b-[10px] p-6 lg:p-8 flex flex-col md:flex-row items-center gap-6 lg:gap-8 transition-transform hover:-translate-y-1 active:translate-y-1 active:border-b-[2px] cursor-pointer shadow-sm relative group overflow-hidden">
              ...
              <div className="shrink-0 relative w-24 h-24 lg:w-32 lg:h-32 bg-sky-50 border-2 border-sky-100 rounded-[2rem] flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform duration-300">
                ...
                activeCourse.imageSrc.startsWith("http") ? (
                  <Image
                  ...
                  <span className="text-6xl drop-shadow-xl inline-block -rotate-6">
                    🚌
                  </span>
                )}
              </div>
              ...
              <div className="flex-1 flex flex-col justify-center items-center md:items-start text-center md:text-left gap-2 w-full">
                <h2 className="text-4xl lg:text-5xl font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight">
                  {activeCourse.title}
                </h2>

                <div className="w-full max-w-sm flex flex-col gap-2 mt-2">
                  <div className="flex-1 h-5 w-full bg-slate-100 dark:bg-slate-800 rounded-full border-2 border-slate-200 dark:border-slate-800 overflow-hidden relative">
  ...
  return (
    <div className="animate-in fade-in duration-500 w-full space-y-10">
      ...
      <div className="flex flex-col gap-4 mb-4">
        <div className="h-6 w-48 bg-stone-200 dark:bg-slate-700 rounded-md animate-pulse pl-2" />
        <div className="w-full bg-white dark:bg-slate-900 rounded-[32px] border-2 border-stone-200 dark:border-slate-800 border-b-[10px] p-6 lg:p-8 flex flex-col md:flex-row items-center gap-6 lg:gap-8 shadow-sm">
          {/* Left: Landmark Illustration Skeleton */}
          <div className="shrink-0 relative w-24 h-24 lg:w-32 lg:h-32 bg-stone-100 dark:bg-slate-800 rounded-[2rem] border-2 border-stone-200 dark:border-slate-800 flex items-center justify-center animate-pulse" />
          ...
          <div className="flex-1 flex flex-col justify-center items-center md:items-start text-center md:text-left gap-4 w-full">
            ...
            <div className="w-full max-w-sm flex flex-col gap-2 mt-2">
              <div className="flex-1 h-5 w-full bg-stone-200 dark:bg-slate-700 rounded-full animate-pulse" />
          ...
          <div className="shrink-0 mt-4 md:mt-0 w-full md:w-auto">
            <div className="w-full md:w-[200px] h-16 rounded-2xl bg-stone-200 dark:bg-slate-700 border-b-[8px] border-stone-300 dark:border-slate-700 animate-pulse" />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="pt-6 grid grid-cols-2 lg:grid-cols-[repeat(auto-fill,minmax(210px,1fr))] gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="pt-6 pb-8 px-3 rounded-2xl border-2 border-stone-200 dark:border-slate-800 border-b-4 bg-stone-50 dark:bg-slate-950 flex flex-col items-center min-h-[220px] animate-pulse"
            >
              <div className="w-[80px] h-[80px] rounded-2xl bg-stone-200 dark:bg-slate-700 mb-6 drop-shadow-md border-b-4 border-stone-300 dark:border-slate-700" />
