  ...
  const generateDeck = useCallback((amount = 10) => {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      if (!isCorrect) {
        ...
        const fakes = VOCAB_DICTIONARY.filter(
          (w) =>
            w.category === randomSource.category && w.en !== randomSource.en,
        );
        if (fakes.length > 0) {
          displayEn = fakes[Math.floor(Math.random() * fakes.length)].en;
        } else {
          // Fallback to any random fake if no peers in category
          const allFakes = VOCAB_DICTIONARY.filter(
            (w) => w.en !== randomSource.en,
          );
          ...
          {deck.slice(-3).map((card, idx, arr) => {
            ...
            return (
              <motion.div
                key={card.id}
                className={cn(
                  "absolute bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-8 rounded-[2rem] w-72 md:w-80 h-96 shadow-xl flex flex-col p-6 overflow-hidden",
                  isTop
              ...
              >
                ...
                <div
                  className="absolute inset-0 bg-emerald-400 opacity-0 transition-opacity flex items-center justify-center -z-0"
                  id="right-indicator"
                ></div>
                <div
                  className="absolute inset-0 bg-rose-400 opacity-0 transition-opacity flex items-center justify-center -z-0"
                  id="left-indicator"
                ></div>
                ...
                <div className="w-full h-1 bg-stone-100 dark:bg-slate-800 rounded-full my-4 z-10" />

                <div className="flex-1 flex items-center justify-center z-10 w-full text-center">
                  <h2 className="text-2xl md:text-3xl font-black text-stone-600 dark:text-slate-300 break-words">
                    {card.pt}
                  </h2>
                </div>
              </motion.div>
