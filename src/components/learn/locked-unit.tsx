...
export function LockedUnit({
  ...
  const isUnitActive =
    !isCompleted && lessons.some((l) => l.completed || l.isCurrent);
  ...
  const nodeBgColor = isCompleted
    ? "bg-[#58cc02] shadow-[0_4px_0_#4da918]"
    : "bg-[#cecece] shadow-[0_4px_0_#b3b3b3]";

  return (
    <div
      className={`rounded-[24px] p-6 w-full max-w-[450px] h-auto sm:h-[180px] relative z-10 ${bgColor} shadow-[0_10px_0_${shadowColor}] ${align === "right" ? "sm:self-end" : "sm:self-start"} mb-8`}
    >
      ...
      <div className="flex justify-between items-start mb-8 relative z-10">
        <div>
          <h2
            className={`text-sm sm:text-lg font-bold ${headerColor} uppercase tracking-wide`}
          >
            {t("unit_number", { number: unitNumber })}
          </h2>
          <p
            className={`font-black text-lg sm:text-xl truncate max-w-[200px] ${titleColor}`}
          >
            {title}
          </p>
        </div>
        ...
        <div className="absolute right-0 top-[-30px] w-32 h-24 items-end justify-end pointer-events-none opacity-50 hidden sm:flex">
          {iconType === "globe" ? (
            <div
              className={`w-16 h-16 ${isCompleted ? "bg-white/20" : "bg-gray-300"} rounded-full flex items-center justify-center`}
            >
              <Globe
                className={`${isCompleted ? "text-white" : "text-gray-400"} w-10 h-10`}
              />
            </div>
          ) : (
            <div className="flex items-end space-x-1">
              <div
                className={`w-4 h-12 ${isCompleted ? "bg-white/20" : "bg-gray-300"} rounded-t-sm`}
              ></div>
              <div
                className={`w-6 h-16 ${isCompleted ? "bg-white/20" : "bg-gray-300"} rounded-t-sm`}
              ></div>
              <div
                className={`w-8 h-10 ${isCompleted ? "bg-white/20" : "bg-gray-300"} rounded-t-sm`}
              ></div>
            </div>
          )}
        </div>
      ...
      <div className="relative w-full h-16 flex items-center justify-center px-4 sm:px-8 gap-6 sm:gap-12 z-10">
        {/* Connecting Line */}
        <div
          className={`absolute top-1/2 left-8 right-8 h-2 ${isCompleted ? "bg-white/40" : "bg-[#cecece]"} -translate-y-1/2 z-0 rounded-full`}
        ></div>
        ...
        {lessons.slice(0, 3).map((lesson, idx) => {
          ...
          return (
            <div
              key={lesson.id}
              className={`relative z-10 w-10 h-10 sm:w-12 sm:h-12 rounded-full ${nodeBgColor} flex items-center justify-center ring-2 ring-white/10`}
            >
              {isCompleted ? (
                <Check
