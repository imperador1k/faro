  ...
  return (
    <div className="rounded-[24px] p-6 w-full max-w-[600px] h-auto sm:h-[250px] relative z-10 text-white mt-10 bg-[#58cc02] shadow-[0_12px_0_#4da918] flex flex-col mb-12">
      <div className="flex justify-between items-start relative z-10">
        <div className="z-10 mt-2">
          <h2 className="text-xl font-bold opacity-90 mb-1 tracking-wide uppercase text-white">
            {t("unit_label", { index: unitIndex + 1 })}
          </h2>
        ...
        <div className="absolute right-0 top-[-40px] w-48 sm:w-64 h-36 items-end justify-end pointer-events-none z-0 overflow-visible hidden sm:flex">
          ...
          <div className="relative w-20 h-20 bg-[#ffe8cd] rounded-full mb-0 mr-0 border-4 border-white/20 shadow-lg z-10 flex items-center justify-center">
            <Cat
              className="text-[#f59e0b] w-10 h-10 drop-shadow-sm"
              fill="currentColor"
            />
          </div>
        </div>
      ...
      <div className="relative w-full flex-1 mt-12 sm:mt-6 min-h-[140px]">
        <svg
          className="absolute inset-x-0 w-[110%] sm:w-full h-full glowing-path z-0 top-0 left-[-5%] sm:left-0"
          ...
          if (lesson.isCurrent) {
            return (
              <div key={lesson.id} className="absolute z-10" style={pos}>
                <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-white dark:bg-slate-900 text-gray-700 px-3 py-1 rounded-[16px] text-sm font-bold shadow-md whitespace-nowrap flex items-center gap-1 z-20 animate-bounce">
                  <Leaf
                    className="text-[#58cc02] w-4 h-4"
                    fill="currentColor"
                  />{" "}
                  +10
                ...
                <Link href={`/lesson/${lesson.id}`}>
                  <div className="w-[60px] h-[60px] rounded-full bg-white dark:bg-slate-900 shadow-[0_4px_0_#e5e5e5] flex items-center justify-center relative cursor-pointer group ring-4 ring-white/30 hover:scale-105 transition-transform">
                    <IconComponent
                      className="text-[#cecece] w-8 h-8 group-hover:text-gray-400 transition-colors"
                      fill="currentColor"
                    />
