...
export const UnitSearchModal = ({ units }: UnitSearchModalProps) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-modal bg-slate-900/40 backdrop-blur-sm flex items-start justify-center pt-[15vh] px-4 animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border-2 border-stone-200 dark:border-slate-800 border-b-[8px] flex flex-col overflow-hidden animate-in zoom-in-95 slide-in-from-top-4 duration-300">
        {/* Header Input */}
        <div className="flex items-center px-4 py-3 bg-stone-50 dark:bg-slate-950 border-b-2 border-stone-200 dark:border-slate-800 shadow-sm relative z-10">
          <Search
            className="w-8 h-8 text-stone-400 dark:text-slate-500 dark:text-slate-400 mr-3 shrink-0"
            strokeWidth={3}
          />
          <input
            ref={inputRef}
        ...
        {/* Results Area */}
        <div className="flex flex-col max-h-[50vh] overflow-y-auto p-4 custom-scrollbar gap-3 bg-[#f3f6f8] dark:bg-slate-950/80">
          {filteredUnits.length === 0 ? (
            ...
            filteredUnits.map((unit) => (
              <div
                ...
                className="group bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-4 rounded-2xl p-4 flex items-center justify-between cursor-pointer hover:-translate-y-1 hover:border-sky-300 dark:hover:border-sky-700 hover:shadow-md active:translate-y-0 active:border-b-2 transition-all"
              >
                ...
                <div className="w-10 h-10 rounded-full bg-stone-100 dark:bg-slate-800 flex items-center justify-center group-hover:bg-sky-100 dark:group-hover:bg-sky-900/40 group-hover:text-sky-500 dark:group-hover:text-sky-400 transition-colors shrink-0">
                  <ChevronRight
                    strokeWidth={3}
                    className="text-stone-400 dark:text-slate-500 dark:text-slate-400 group-hover:text-sky-500 transition-colors"
                  />
                </div>
              </div>
            ))
