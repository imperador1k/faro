    ...
    <div className="w-full relative flex flex-col items-center mb-8">
      ...
      <div
        className={cn(
          ...
          isLockedUnit
            ? "bg-stone-200 dark:bg-slate-700 border-2 border-stone-300 dark:border-slate-700 border-b-8 rounded-3xl opacity-80"
      ...
      >
        ...
        <h3
          className={cn(
            ...
            isLockedUnit
              ? "text-stone-500 dark:text-slate-400 text-xl md:text-2xl"
              : "text-white text-2xl md:text-3xl drop-shadow-sm",
          )}
        >
          {title}
        </h3>
      ...
      <div
        className={cn(
          ...
          !isLockedUnit
            ? "bg-white dark:bg-slate-900 border-x-2 border-b-2 border-[#e5e7eb] dark:border-slate-800 rounded-b-3xl shadow-[0_10px_30px_rgba(0,0,0,0.05)] pb-10"
            : "pb-10",
        )}
        style={{
        ...
        <div className="absolute inset-0 z-10 pointer-events-none">
          ...
          {isLockedUnit && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-auto z-10">
              <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-[2rem] p-6 shadow-xl border-4 border-white/50 dark:border-slate-800/50 flex flex-col items-center justify-center -translate-y-12">
              ...
              >
                  ...
                  <div className="relative group cursor-pointer z-40 flex flex-col items-center justify-center">
                    ...
                    >
                      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center relative z-10">
                        <IconComponent
                          className="w-7 h-7 sm:w-9 sm:h-9 text-white drop-shadow-sm"
                          strokeWidth={3}
                          fill="currentColor"
                        />
                      </div>
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 flex flex-col items-center mt-3 pointer-events-none">
                      ...
                      <div className="opacity-100 group-hover:translate-y-1 transition-transform z-30 pointer-events-auto">
                        <div
                          className="bg-white dark:bg-slate-900 px-3 py-1.5 rounded-2xl text-[11px] font-black tracking-widest uppercase shadow-md flex items-center gap-1.5 border-2 border-stone-200 dark:border-slate-800 whitespace-nowrap cursor-default"
                ...
                {lesson.completed && !lesson.isCurrent && (
                  <div className="relative group cursor-pointer z-20 flex flex-col items-center">
                    ...
                    >
                      <Check
                        className="text-white w-8 h-8 sm:w-10 sm:h-10 drop-shadow-sm"
                        strokeWidth={5}
                      />
                    </div>
                  </div>
                ...
                {lesson.isLocked &&
                  ...
                  !lesson.isCurrent &&
                  !lesson.completed && (
                    <div className="w-[64px] h-[64px] sm:w-[72px] sm:h-[72px] bg-stone-200 dark:bg-slate-700 rounded-full border-4 border-white dark:border-slate-800 shadow-sm border-b-[6px] border-b-stone-300 dark:border-b-slate-800 flex flex-col items-center justify-center cursor-not-allowed">
                      <IconComponent
                        className="text-stone-400 dark:text-slate-500 dark:text-slate-400 w-6 h-6 sm:w-8 sm:h-8 drop-shadow-sm"
                        strokeWidth={3}
                        fill="currentColor"
                      />
                    </div>
