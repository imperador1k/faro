...
export const AchievementsList = ({ achievements, userProgress }: Props) => {
    ...
    return (
      <div className="mb-10" key={title}>
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 bg-stone-100 dark:bg-slate-800 rounded-xl border-2 border-stone-200 dark:border-slate-800 border-b-4">
            <CategoryIcon className="h-5 w-5 text-stone-600 dark:text-slate-300" />
          </div>
          <h3 className="text-xl font-black text-stone-700 dark:text-slate-200 uppercase tracking-tight">
            {title}
          </h3>
        </div>
              ...
              <div
                ...
                className={cn(
                  ...
                  achievement.unlocked
                    ? "border-amber-200 dark:border-amber-900/50 bg-white dark:bg-slate-900 border-b-8 shadow-sm hover:shadow-md hover:-translate-y-1 active:translate-y-0 active:border-b-0 active:mb-[8px]"
                    : "border-stone-200 dark:border-slate-800 bg-stone-50/50 dark:bg-slate-950/50 border-b-8 grayscale opacity-70",
                )}
              >
                <div
                  className={cn(
                    ...
                    achievement.unlocked
                      ? "bg-amber-50 dark:bg-amber-950/30 border-amber-100 dark:border-amber-900/50 border-b-4 text-amber-500 group-hover:bounce"
                      : "bg-stone-200 dark:bg-slate-700 border-stone-300 dark:border-slate-700 text-stone-400 dark:text-slate-500 dark:text-slate-400",
                  )}
                >
                  {achievement.icon}
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm font-bold text-stone-400 dark:text-slate-500 dark:text-slate-400 truncate mb-3">
                    {achievement.description}
                  </p>

                  {!achievement.unlocked && (
                    <div className="w-full h-4 rounded-full bg-stone-200 dark:bg-slate-700 overflow-hidden mt-1 relative border border-stone-300 dark:border-slate-700 shadow-inner">
                      <div
  ...
  return (
    <div className="mb-10 w-full animate-in fade-in duration-700 delay-300">
      <div className="mb-8 flex flex-col sm:flex-row items-center justify-between border-b-4 border-stone-100 dark:border-slate-800 pb-6 gap-6 sm:gap-4">
        <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-4">
          <div className="h-14 w-14 bg-amber-100 dark:bg-amber-950/30 rounded-2xl flex items-center justify-center border-2 border-amber-200 dark:border-amber-900/50 border-b-4 shadow-sm shrink-0">
