      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between mb-10 gap-4">
          <div className="flex items-center gap-4">
            <div className="p-4 bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-4 rounded-2xl shadow-sm transform -rotate-3">
              {icon}
            </div>
            <div>
              <h1 className="text-3xl md:text-4xl font-black text-stone-700 dark:text-slate-200 tracking-tight">
                {title}
              </h1>
...
export const LegalSection = ({
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-8 rounded-3xl p-6 md:p-8 shadow-sm ${className || ""}`}
    >
      <h2 className="text-2xl font-black text-[#1CB0F6] mb-4">{title}</h2>
      <div className="text-lg text-stone-600 dark:text-slate-300 leading-relaxed font-medium space-y-4">
        {children}
      </div>
    </div>
  );
