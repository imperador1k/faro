...
export const AIGeneratorForm = ({ courseId, targetLang }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    toast.loading("A Coruja está a pensar... Isto pode demorar.", {
      id: "generating",
    });
    ...
    try {
      ...
      if (result && result.error) {
        ...
        toast.error("Oops! Algo correu mal.");
      ...
      toast.dismiss("generating");
      toast.success("✨ Sucesso! Unidades, lições e desafios gerados com IA.", {
        duration: 5000,
      });
        ...
        <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="text-sm text-violet-700">
            ...
            <p className="text-violet-600 mt-1">
              1 Unidade → 3 Lições → 4-5 Desafios cada (SELECT, INSERT, MATCH,
              DICTATION) com opções e explicações pedagógicas.
            </p>
          </div>
        </div>
        ...
        {loading && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 flex flex-col items-center gap-3 animate-pulse">
            ...
            <p className="text-amber-500 text-xs font-medium">
              Gemini 2.5 Flash • Não feches esta página.
            </p>
          </div>
        )}
