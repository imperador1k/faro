...
const strengthInfo: Record<
  number,
  { label: string; color: string; pct: number }
> = {
  1: {
    label: "Semente",
    color: "text-amber-600 bg-amber-100 border-amber-200 stroke-amber-500",
    pct: 25,
  },
  2: {
    label: "Broto",
    color: "text-lime-600 bg-lime-100 border-lime-200 stroke-lime-500",
    pct: 50,
  },
  3: {
    label: "Árvore",
    color:
      "text-emerald-600 bg-emerald-100 border-emerald-200 stroke-emerald-500",
    pct: 75,
  },
  4: {
    label: "Mestre",
    color: "text-indigo-600 bg-indigo-100 border-indigo-200 stroke-indigo-500",
    pct: 100,
  },
};
...
export const WordCard = ({
  ...
  const cycleLength = 6;
  const offsetCalc = Math.sin(
    (index % cycleLength) * (Math.PI / (cycleLength / 2)),
  );
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-900 border-2 border-stone-200 dark:border-slate-800 border-b-[8px] rounded-[32px] p-6 sm:p-8 w-full shadow-sm hover:shadow-md transition-all duration-300 hover:-translate-y-1 relative overflow-hidden">
        {/* ── Top Bar ── */}
        <div className="flex items-center justify-between mb-4">
          <div
            className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border-2 border-transparent border-b-[4px] shadow-sm ${info.color.replace("stroke-", "text-")}`}
          >
        ...
        {isRevealed ? (
          <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
            {/* Definition Block */}
            {explanation && (
              <div className="bg-stone-50 dark:bg-slate-950 border-2 border-stone-100 rounded-2xl p-4 flex gap-3">
                <Info className="w-5 h-5 text-stone-400 dark:text-slate-500 dark:text-slate-400 shrink-0 mt-0.5" />
                <p className="text-sm font-bold text-stone-500 dark:text-slate-400 leading-relaxed">
                  {explanation}
                </p>
              </div>
            )}
                    ...
                    {contextSentence
                      ...
                      .map((part, i) =>
                        part.toLowerCase() === word.toLowerCase() ? (
                          <strong
                            key={i}
                            className="font-black bg-sky-200 px-1 rounded mx-0.5 text-sky-900 border-b-2 border-sky-300"
                          >
                            {part}
                          </strong>
                        ) : (
          ...
          <div
            ...
            className="w-full bg-stone-50 dark:bg-slate-950 border-2 border-stone-200 dark:border-slate-800 border-dashed rounded-2xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-stone-100 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors text-stone-400 dark:text-slate-500 dark:text-slate-400 hover:text-stone-500 dark:text-slate-400 group/reveal"
          >
        ...
        <div className="absolute -bottom-8 -right-8 opacity-10 pointer-events-none">
          <svg className="w-48 h-48 -rotate-90">
            <circle
              ...
              cy="96"
              r="80"
              className="fill-none stroke-stone-300 stroke-[24]"
            />
            <circle
              cx="96"
              cy="96"
      ...
      <div className="w-2 md:w-4 h-12 md:h-16 bg-stone-200 dark:bg-slate-700 opacity-50 relative -z-10 mt-1 mb-1 rounded-full mx-auto" />
    </div>
  );
};
