...
export const InteractiveText = ({
  ...
  const handleSaveToVault = async () => {
    if (!selectedWord || !translationData || savedWords.has(selectedWord))
      return;
    ...
    try {
      ...
      setTranslationData((prev) =>
        prev ? { ...prev, isAlreadySaved: true } : prev,
      );
  ...
  const isWordSaved = selectedWord
    ? savedWords.has(selectedWord) || (translationData?.isAlreadySaved ?? false)
    : false;
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative inline leading-relaxed", className)}
    >
      {tokens.map((token, i) => {
        ...
        <>
          ...
          <div
            ...
            className={cn(
              ...
              "bg-white dark:bg-slate-900 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.12)]",
              "p-6 pb-8",
...
const OverlayContent = ({
  ...
  <div>
    ...
    {isLoading ? (
      ...
      <>
        ...
        <Button
          ...
          className={cn(
            ...
            isSaved
              ? "bg-slate-100 dark:bg-slate-800 text-slate-400 border border-slate-200 dark:border-slate-800 cursor-default hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800"
      ...
      <p className="text-sm font-medium text-slate-500 dark:text-slate-400 py-4 text-center">
