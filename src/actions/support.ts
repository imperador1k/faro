...
const supportSchema = z.object({
  ...
  message: z
    .string()
    .min(10, "A mensagem tem de ter pelo menos 10 caracteres."),
});
...
export async function submitSupportTicket(
  ...
  if (honeypot) {
    console.warn(
      "[SECURITY] Spam Bot detetado a atacar a página de Suporte. Ticket descartado silenciosamente.",
    );
  ...
  if (!user) {
    return {
      message:
        "Precisas de ter sessão iniciada para enviar pedidos de suporte.",
      errors: { message: ["Acesso não autorizado"] },
    };
  }

  const userEmail =
    user.emailAddresses[0]?.emailAddress || "sem_email@user.com";
  ...
  if (!targetEmail) {
    return {
      message:
        "O email de suporte destino não está listado nas configurações do sistema.",
      errors: { message: ["Erro de configuração no servidor"] },
    };
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Email send failed but ticket was saved to DB:",
        emailError,
      );
    ...
    return {
      message:
        "Ocorreu um erro ao processar o teu pedido na Base de Dados. Verifica se a tabela existe.",
