...
async function uploadCourseImage(file: File): Promise<string> {
  if (!validateFileType(file, ALLOWED_IMAGE_TYPES)) {
    throw new Error(
      "Tipo de ficheiro não suportado. Apenas JPG, PNG, WebP e GIF são permitidos.",
    );
  }
  ...
  try {
    if (id) {
      ...
      await logAdminAction(
        "UPDATE_COURSE",
        id,
        JSON.stringify({ title, language, languageCode }),
      );
    } else {
      if (!imageSrc) {
        return actionError(
          "BAD_REQUEST",
          "Imagem é obrigatória ao criar um novo curso.",
        );
      }
    ...
    return actionError(
      "SERVER_ERROR",
      "Erro ao guardar o curso. Tenta novamente.",
    );
  }
