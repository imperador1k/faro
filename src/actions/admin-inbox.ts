...
export async function getInboxItems(
  ...
  try {
    ...
    const reviews = await db.query.userReviews.findMany({
      where: showArchived
        ? and(
            eq(userReviews.read, true),
            gte(userReviews.createdAt, sevenDaysAgo),
          )
...
export async function replyToTicket(
  ...
  if (!rateLimitResult.success) {
    return actionError(
      "RATE_LIMIT_EXCEEDED",
      "Demasiados emails enviados. Aguarda antes de tentar novamente.",
    );
  }
  ...
  try {
    ...
    const htmlContent = getSupportReplyEmail(
      sanitizedName,
      sanitizedContent,
      sanitizedSubject,
    );
    ...
    await resend.emails.send({
      from: "Equipa Faro <suporte@miguelweb.dev>",
